...
@click.argument("user_id")
@click.option("--cursor", default="", help="Pagination cursor")
@click.option("--detail", is_flag=True, help="Fetch full content for each note")
@structured_output_options
@click.pass_context
def user_posts(ctx, user_id: str, cursor: str, detail: bool, as_json: bool, as_yaml: bool):
    """List a user's published notes."""
    def _user_posts_action(client):
        if detail:
            return client.get_user_notes_with_detail(user_id)
    ...
    def _render_user_posts(data):
        page = normalize_paged_notes(data)
        if detail:
            _render_detailed_notes(page["notes"])
        else:
...
def _render_detailed_notes(notes):
    """Render notes with full content."""
    for i, note in enumerate(notes, 1):
        title = note.get("display_title", note.get("title", "untitled"))
        desc = note.get("desc", "")
        note_id = note.get("note_id", note.get("id", ""))
        interact = note.get("interact_info", {})
        liked = note.get("liked_count", interact.get("liked_count", "?"))
        comment_count = interact.get("comment_count", "")
        share_count = interact.get("share_count", "")
        tags = note.get("tag_list", [])
        tag_names = " ".join(f"#{t.get('name', '')}" for t in tags[:5]) if tags else ""

        print_info(f"{'='*60}")
        print_info(f"[{i}] {title}")
        print_info(f"    ID: {note_id}")
        stats = f"    ❤️ {liked}"
        if comment_count:
            stats += f" | 💬 {comment_count}"
        if share_count:
            stats += f" | 🔗 {share_count}"
        print_info(stats)
        if tag_names:
            print_info(f"    {tag_names}")
        if desc:
            print_info(f"")
            print_info(f"    {desc}")
        print_info("")
